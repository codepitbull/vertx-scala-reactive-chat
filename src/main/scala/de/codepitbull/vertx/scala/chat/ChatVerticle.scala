...
class ChatVerticle extends ScalaVerticle {
  ...
  override def startFuture(): Future[_] = {
    val port = config.getInteger("port", 8044)
    val sender = vertx.eventBus().publisher[JsonObject]("browser")
    val consumer = vertx.eventBus().consumer[String]("server")
      .handler(msg => sender
        .write(Json.obj(("msg",msg.body()), ("date", Calendar.getInstance().getTime().toString))))
